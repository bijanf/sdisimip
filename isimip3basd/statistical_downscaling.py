...
def initializer(q, l, m, a):
    """
    Sets global variables global_i_archive, global_lazy_data,
    global_month_numbers, global_done_already to q.get(), l, m, a,
    ...
    a : array
        Array indicating locations that are done already.
...
def downscale_one_location(
    ...
    **kwargs : Passed on to downscale_one_month.
    ...
    if all([done_already[i_loc_1d(i)] for i in i_locs]):
    ...
    if np.isnan(if_all_invalid_use):
        if uf.only_missing_values_in_at_least_one_time_series(data):
            ...
            sys.stdout.flush()
            a = np.expand_dims(np.repeat(np.float32(fill_value),
                month_numbers['sim_coarse'].size), axis=1)
            for i_loc in i_locs:
                if not done_already[i_loc_1d(i_loc)]:
                    uf.to_npy_stack(i_loc_1d(i_loc), i_archive, sim_fine_path,a)
            return None
    ...
    for i, i_loc in enumerate(i_locs):
        if not done_already[i_loc_1d(i_loc)]:
            uf.to_npy_stack(i_loc_1d(i_loc), i_archive, sim_fine_path,
                np.expand_dims(sim_fine[:,i].data, axis=1))
...
def downscale(
        ...
        randomization_seed=None, resume_job=False, **kwargs):
    """
    Applies the modified MBCn algorithm for statistical downscaling calendar
    ...
    resume_job : boolean, optional
        If True, use local statistical downscaling results where they exist.
        If False, overwrite existing local statistical downscaling results.
    ...
    **kwargs : Passed on to downscale_one_location.
    ...
    print('----the shape of the comparisons------') # Bijan 
    print('obs_fine.shape[1:], sim_coarse.shape[1:] ==',
     obs_fine.shape[1:], sim_coarse.shape[1:]) #Bijan 
    ...
    # prepare loading local downscaling results using uf.from_npy_stack
    done_already = uf.setup_npy_stack(sim_fine_path, n_processes, 
        np.prod(np.array(obs_fine.shape[1:])), resume_job)
    ...
    if n_processes > 1:
        pool = mp.Pool(n_processes, initializer=initializer, initargs=(
            i_archive_queue, lazy_data, month_numbers, done_already))
        foo = list(pool.imap(sdol, i_locations_coarse))
        pool.close()
        pool.join()
        pool.terminate()
    else:
        initializer(i_archive_queue, lazy_data, month_numbers, done_already)
...
def main():
    ...
    downscale(
        obs_fine, sim_coarse, sim_coarse_remapbil,
        options.sim_fine, options.n_processes, options.n_iterations,
        options.randomization_seed, options.resume_job,
    ...
    d = uf.from_npy_stack(options.sim_fine, options.n_processes, sim_fine.shape)
    ...
    if not options.keep_npy_stack:
        shutil.rmtree(uf.npy_stack_dir(options.sim_fine))
