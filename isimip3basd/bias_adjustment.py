...
global_years = {}
global_doys = {}
global_done_already = None



def initializer(q, l, m, y, d, a):
    """
    Sets global variables global_i_archive, global_lazy_data,
    global_month_numbers, global_years, global_doys, global_done_already to
    q.get(), l, m, y, d, a, respectively. These global variables are used as
    shared resources by all processes in the pool for n_processes > 1. Using
    these shared resources drastically reduces the amount of data that needs
    to be piped to the processes at the beginning of every local bias
    adjustment.

    Parameters
    ----------
    q : multiprocessing manager queue
        Archive indices are taken from this queue.
    ...
    a : (m,n) array
        Array indicating locations that are done already for m locations and n
        variables.

    """
    global global_i_archive
    global global_lazy_data
    global global_month_numbers
    global global_years
    global global_doys
    global global_done_already
    global_i_archive = q.get()
    global_lazy_data = l
    global_month_numbers = m
    global_years = y
    global_doys = d
    global_done_already = a
...
def adjust_bias_one_location(
    ...
    **kwargs : Passed on to adjust_bias_one_month.
    ...
    if np.all(done_already[i_loc_1d]):
    ...
    if uf.only_missing_values_in_at_least_one_dataset(data):
        ...
        a = np.expand_dims(np.repeat(np.float32(fill_value), n_times), axis=1)
        for i, d in enumerate(done_already[i_loc_1d]):
            if not d:
                uf.to_npy_stack(i_loc_1d, i_archive, sim_fut_ba_path[i], a)
    ...
    for i, d in enumerate(done_already[i_loc_1d]):
        if not d:
            uf.to_npy_stack(i_loc_1d, i_archive, sim_fut_ba_path[i],
...
def adjust_bias(
    ...
    resume_job : boolean, optional
        If True, use local bias adjustment results where they exist. If False,
    ...
    **kwargs : Passed on to adjust_bias_one_location.
    ...
    # prepare loading local bias adjustment results using uf.from_npy_stack
    n_variables = len(halfwin_upper_bound_climatology)
    n_space = np.prod(np.array(space_shape))
    done_already = np.empty((n_space, n_variables), dtype=bool)
    for i, p in enumerate(sim_fut_ba_path):
        done_already[:,i] = uf.setup_npy_stack(
            p, n_processes, n_space, resume_job)
    ...
    # prepare queue that enables every process to write to its own npz archive
    manager = mp.Manager()
    i_archive_queue = manager.Queue()
    for i in range(n_processes):
        i_archive_queue.put(i)

    # adjust every location individually
    i_locations = np.ndindex(space_shape)
    abol = partial(adjust_bias_one_location,
        sim_fut_ba_path=sim_fut_ba_path, space_shape=space_shape,
        halfwin_upper_bound_climatology=halfwin_upper_bound_climatology,
        detrend=detrend, rotation_matrices=rotation_matrices, 
        randomization_seed=randomization_seed, step_size=step_size,
        window_centers=window_centers, **kwargs)
    print('adjusting at location ...')
    if n_processes > 1:
        pool = mp.Pool(n_processes, initializer=initializer, initargs=(
            i_archive_queue, lazy_data, month_numbers, years, doys,
            done_already))
        ...
        initializer(i_archive_queue, lazy_data, month_numbers, years, doys,
            done_already)
...
def main():
    ...
    adjust_bias(
        ...
        options.step_size, options.resume_job,
        months=months,
    ...
    for i, v in enumerate(variable):
        ...
        d = uf.from_npy_stack(sim_fut_ba_path[i], options.n_processes,
            sim_fut_ba.shape)
        ...
        if not options.keep_npy_stack:
            shutil.rmtree(uf.npy_stack_dir(sim_fut_ba_path[i]))
